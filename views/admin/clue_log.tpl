...
{% block content %}
<div class="content-title"><a href="/admin/clue">返回线索列表 >></a></div>
<div class="content-mainer">
  {% if userInfo.role == 1 %}
  <div class="clue-section">
    <div class="form-item">
      <p class="form-text">客户名称：{{clue.name}}</p>
    </div>
    <div class="form-item">
      <p class="form-text">联系电话：{{clue.phone}}</p>
    </div>
    <div class="form-item">
      <p class="form-text">线索来源：{{clue.utm}}</p>
    </div>
    <div class="form-item">
      <p class="form-text">创建时间：{{clue.created_timle_display}}</p>
    </div>
    <div class="form-item">
      <p class="form-text">用户状态：</p>
    ...
    <div class="form-item">
      <p class="form-text">当前分配销售：</p>
      <select id="clueUserId" class="form-input">
        <option value="">请选择分配销售</option>
        {% for val in users  %}
...
<input type="text" id="clueId" value="{{clue.id}}" hidden>
